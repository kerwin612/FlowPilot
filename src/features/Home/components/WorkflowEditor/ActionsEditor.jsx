 ...
 * 可排序动作器项
 */
const SortableActionItem = memo(({ act, index, onToggle, onRemove, onConfigChange, onTitleChange, onConditionChange, manualByKey }) => {
  ...
  const title = act.title || def?.label || act.key
  const subTitle = def?.label || act.key
          ...
          <Space style={{ width: '100%' }} direction="vertical" size={12}>
            <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
              ...
              <Input
                ...
                value={act.title || ''}
                onChange={(e) => onTitleChange(act.id, e.target.value)}
                style={{ flex: 1 }}
              />
            </div>
