      ...
      items: [
        {
          ...
          iconColor: '#1890ff',
          feature: { enabled: true, code: 'wf-page-app-showcase-linux', explain: '展示页面应用动作能力（完整HTML）', cmds: ['页面应用演示','PageApp'] },
          executors: [
            {
              key: 'param-builder',
              enabled: true,
        ...
        },
        {
          ...
          executors: [
            ...
            },
            {
              ...
              enabled: true,
              config: { 
                template: 'gedit {{executors[0].result.value.filePath}}' 
              }
            }
          ],
          actions: []
  ...
  return { value: { file: path, kind: isZip ? 'zip' : (isText ? 'text' : (isImage ? 'image' : 'other')) } };
              ...
              executors: [
                ...
                },
                {
                  ...
                  config: { template: 'gedit {{executors[0].result.value.file}}' }
                }
              ],
              actions: [
                {
            ...
            
            {
              ...
              executors: [
                ...
                },
                {
                  ...
                  enabled: true,
                  config: { 
                    template: 'tar -xzf {{executors[0].result.value.archive}} -C {{executors[0].result.value.dest}}' 
                  }
                }
              ],
              actions: [
                {
                  key: 'open-path',
                  enabled: true,
                  config: { path: '{{executors[0].result.value.dest}}' }
                }
              ]
            },
            {
